    ...
    try:
        ...
        try:
            call = twilio_client.calls.create(
                ...
                url=f"{telephony_host}/twilio_connect?nexa_host={nexa_host}&agent_id={agent_id}",
                method="POST",
                record=True
            )
...
async def twilio_connect(nexa_host: str = Query(...), agent_id: str = Query(...)):
    try:
        response = VoiceResponse()

        connect = Connect()
        nexa_websocket_url = f'{nexa_host}/chat/v1/{agent_id}'
        connect.stream(url=nexa_websocket_url)
        print(f"websocket connection done to {nexa_websocket_url}")
